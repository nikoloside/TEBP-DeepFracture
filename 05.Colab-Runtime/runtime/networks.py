"""DeepFracture network definitions.

Mirrors TEBP-DeepFracture 04.Run-time/predict/Model/load_VQfinal2resolutionv2.py.
The released checkpoints on hf.co/nikoloside/deepfracture are full pickled
modules saved from __main__, so `register_for_unpickle()` must be called
before `torch.load`.
"""
...
class AutoDecoder(nn.Module):
    def __init__(self, opt):
        ...
        self.fc = nn.Sequential(
            nn.Linear(opt.pos_encode_dim + opt.z_latent_dim,
                      int((self.ndf * 8) * int(self.data_shape / 16) ** 3)),
    ...
    def forwardMiddle(self, input_x):
        feature = self.fc(input_x).reshape(1, self.ndf * 8,
                                           int(self.data_shape / 16),
                                           int(self.data_shape / 16),
                                           int(self.data_shape / 16))
...
def register_for_unpickle():
    """The checkpoints were pickled with classes living in __main__."""
    main = sys.modules["__main__"]
    main.MultiLatentEncoder = MultiLatentEncoder
    main.AutoDecoder = AutoDecoder
